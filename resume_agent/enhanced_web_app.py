...
class EnhancedResumeAnalyzer:
    ...
    def extract_personal_info(self, text: str) -> Dict:
        ...
        for pattern in name_patterns:
            ...
            if matches:
                personal_info['name'] = matches[0].strip().title()
                break
...
def index():
    ...
    return render_template('enhanced_index.html', results=all_results)
...
def candidate_detail(filename):
    """Show detailed information for a specific candidate"""
    ...
    for file_path in file_paths:
        if os.path.exists(file_path):
            ...
            if result:
                return render_template('candidate_detail.html', candidate=result)
...
def export_results():
    ...
    # Save to CSV
    filename = 'detailed_resume_results.csv'
