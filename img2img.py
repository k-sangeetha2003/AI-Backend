...
def post_process_for_face_similarity(original, generated, faces, face_fidelity=0.9):
    ...
    for face in faces:
        try:
            # Extract face coordinates and ensure they're Python integers, not numpy arrays
            if isinstance(face, np.ndarray):
                # Convert each value to a Python int explicitly
                x = int(face[0])
                y = int(face[1])
                w = int(face[2])
                h = int(face[3])
            else:
                x, y, w, h = map(int, face)  # Convert tuple to integers
            
            # Make sure w and h are positive integers
            if w <= 0 or h <= 0:
            ...
            # Explicitly calculate blur radius as Python int
            blur_radius = max(1, int(w//10))  # Force to Python int
            blend_mask = blend_mask.filter(ImageFilter.GaussianBlur(radius=blur_radius))
            
            # Blend based on face_fidelity parameter
...
async def generate_image(
    ...
    try:
        ...
        # After loading the pipeline with: pipe = load_pipeline(model_path)
        # Add this code:

        # Check if pipeline is actually a text-to-image pipeline
        if not hasattr(pipe, "components"):
            logger.error("Pipeline doesn't have components attribute - cannot convert")
            raise ValueError("Invalid pipeline type - missing components")

        # More reliable pipeline type check
        pipeline_params = list(inspect.signature(pipe.__call__).parameters.keys())
        logger.info(f"Pipeline parameters: {pipeline_params}")

        if "image" not in pipeline_params:
            logger.warning("Detected text-to-image SDXL pipeline - converting to img2img pipeline")
            
            try:
                # Get the components from the text2img pipeline
                from diffusers import StableDiffusionXLImg2ImgPipeline
                
                # Convert the pipeline properly
                ...
                pipe = img2img_pipe
                logger.info("Successfully converted text2img pipeline to img2img")
                
                # Double-check that conversion worked
                if "image" not in inspect.signature(pipe.__call__).parameters:
                    raise ValueError("Pipeline conversion failed - still can't accept image parameter")
                    
            except Exception as conversion_error:
                logger.error(f"Pipeline conversion failed: {conversion_error}")
                # Fall back to a basic image enhancement as last resort
                result = enhance_image_quality(input_image)
                logger.warning("Using basic enhancement instead of AI generation due to pipeline conversion failure")
                
                # Skip the rest of the generation code
                output_path = OUTPUT_DIR / f"{image_id}_fallback_output.png"
                ...
                return {
                     "success": True, 
                    "message": "Image enhanced using fallback method due to model compatibility issues",
                    "url": f"/outputs/{output_path.name}",
                    "image_data_base64": f"data:image/png;base64,{encoded}",
                    "faces_detected": len(faces)
                }
        else:
            logger.info(f"Using img2img pipeline type: {type(pipe).__name__}")
...
async def face_preserving_generate(
    ...
    try:
        ...
        # If prompt-based changes are desired, use higher strength
        strength = min(max(0.55, strength), 0.7)  # Allow more changes while staying safe
        face_fidelity = min(face_fidelity, 0.8)   # Reduce face fidelity slightly

        # Simplify face-preserving prompt to allow main prompt more influence
        face_preserve_prompt = f"{prompt}, preserve facial features"  # Less aggressive preserving terms

        logger.info(f"Using balanced settings: strength={strength}, face_fidelity={face_fidelity}")
        ...
        if len(faces) == 0:
            ...
            # Create an enhanced precision face mask with smaller padding
            face_mask = create_enhanced_face_mask(input_image, faces, padding=40, feather=20)  # Reduced from 80,30
            
            # Allow more strength for transformation while still preserving identity
            strength = min(strength, 0.55)  # Increased from 0.3
...
def test_model_loading(model_id: str):
    ...
    try:
        ...
        # Direct check for pipeline type
        if "StableDiffusionXLPipeline" in str(type(pipe)):
            logger.warning("Detected SDXL text-to-image pipeline directly!")
            
            from diffusers import StableDiffusionXLImg2ImgPipeline
              
            # Force convert to img2img pipeline
            components = pipe.components if hasattr(pipe, "components") else None
            
            if components:
                logger.info("Converting pipeline using components...") 
                img2img_pipe = StableDiffusionXLImg2ImgPipeline(**components)
                img2img_pipe.to("cpu")
                pipe = img2img_pipe
            else:
                # Alternative approach if components aren't available
                logger.warning("Components not available, loading standard img2img pipeline")
            ...
            logger.info(f"Final pipeline type: {type(pipe).__name__}")
